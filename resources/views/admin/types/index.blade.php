...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">All Types</h2>
        <div class="flex items-center gap-4">
            <form action="{{ route('admin.types.index') }}" method="GET" class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search types..."
                    ...
                    @forelse ($types as $type)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $type->id }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $type->name }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-3">
                                    ...
                                    <button type="button"
                                        @click="deleteModalOpen = true; deleteAction = '{{ route('admin.types.destroy', $type) }}'"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-slate-500 dark:text-slate-400">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-lg font-medium text-slate-900 dark:text-white">No types found</p>
