        ...
        <div class="bg-white shadow-md rounded my-6">
            <table class="min-w-full table-auto">
                <thead class="bg-gray-200">
                    <tr>
                        <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-xs leading-4 font-medium text-gray-600 uppercase tracking-wider">Picture</th>
                        ...
                        <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-xs leading-4 font-medium text-gray-600 uppercase tracking-wider">Author</th>
                    ...
                    @foreach ($books as $book)
                        <tr>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                @if($book->picture)
                                    <img src="{{ asset('storage/' . $book->picture) }}" alt="{{ $book->name }}" class="h-10 w-10 rounded-full">
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $book->name }}</td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $book->author }}</td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ ucfirst($book->status) }}</td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $book->type->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
